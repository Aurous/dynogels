...
internals.docClient = DynamoDBDocumentClient.from(dynogels.dynamoDriver);

internals.createSet = (value) => {
  if (_.isArray(value)) return internals.docClient.createSet(value);
  return internals.docClient.createSet([value]);
};

const serialize = internals.serialize = {
  binary: function (value) {
    if (_.isString(value)) return utils.strToBin(value);
    return value;
  },
  date: function (value) {
...
internals.serializeAttribute = serializer.serializeAttribute = (value, type) => {
  if (!type) return value; // if type is unknown, possibly because its an dynamic key return given value
