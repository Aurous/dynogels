...
module.exports = function(credentials) {
  ...
  Table.prototype.sendRequest = async function (method, params, callback) {
    ...
    if (params.RequestItems) {
      for (const key in Object.keys(params.RequestItems)) {
        Object.values(params.RequestItems)[key].Keys = await Object.values(params.RequestItems)[key].Keys.map((i) => marshall(i, marshallOptions));
      }
    }
    ...
    try {
      ...
      if (data.Responses) {
        for await (const key of Object.keys(data.Responses)) {
          data.Responses[key] = data.Responses[key].map((i) => unmarshall(i));
        }
      }
