...
Table.prototype.sendRequest = async function (method, params, callback) {
  ...
  if (params.RequestItems) {
    Object.keys(params.RequestItems).forEach((value, key) => {
      Object.values(params.RequestItems)[key].Keys = Object.values(params.RequestItems)[key].Keys.map((i) => marshall(i, marshallOptions));
    });
  }
  ...
  try {
    const data = await driver.send(fn);
